...
\title{\text{5.Taro Imamura (2005a).tex}}
...
\section{Multiple Scale Expansion}
...
\begin{equation}\begin{aligned}
  ...
  \end{aligned}\end{equation}
\noindent Finally, we can get the $\textbf{the first scale non-equibilirium distribution function}$
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
The distribution function is : (could be controlled by macroscopic variables)
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
where Knuson number is 1, and general distribution function can be calculate as above equation.
\paragraph{Boundary Condition}
\noindent In GILBM, we use macroscopic parameter limitations to enforce boundary conditions on the distribution function. 
At boundary nodes (i.e., nodes located between solid and fluid nodes), we make assumptions to construct the governing equations.
\begin{equation}\begin{aligned}
  \left.f_{\alpha}\right|_{b.c.} &= f_{\alpha}^{eq}  + f_{\alpha}^{(1)}\\
  &= f_{\alpha}^{eq} + \omega \delta t f_{\alpha}^{eq} \left(\frac{(c_{\alpha}^{i}  - u_{i})(c_{\alpha}^{\beta} - u_{\beta})}{c_s^{2}}\frac{\partial u_{i}}{\partial x_{\beta}} - \frac{\partial u_{\beta}}{\partial x_{\beta}}\right) 
\end{aligned}\end{equation}
where gradient of velocity field : 
\begin{equation}\begin{aligned}
  \frac{\partial u_{i}}{\partial x_{\beta}} = \frac{\partial u_{i}}{\partial \xi_{i}} \frac{\partial \xi_{i}}{\partial x_{\beta}}
\end{aligned}\end{equation}
The first term is calculated using second order one side finite difference method : 
\begin{equation}\begin{aligned}
  \left.\frac{\partial u}{\partial \xi}\right|_{i+\frac{1}{2}} = \frac{3u_{i}-4u_{i-1}+u_{i-2}}{2\Delta \xi} + O(\Delta t^{2}) \\[1.5ex]
  \left.\frac{\partial u}{\partial \xi}\right|_{i-\frac{1}{2}} = \frac{-3u_{i}+4u_{i+1}-u_{i+2}}{2\Delta \xi} + O(\Delta t^{2})
 \end{aligned}\end{equation}
 and second is the term related to the curvature, have been calculated in initialization. 

\section{Algorithm}
The contravariant velocity (i.e., the discrete velocity set in curvilinear coordinates) needs to be calculated at the beginning of the simulation process and requires an extra array in the code to store the data. However, this pre-calculated value can reduce the computational cost at every time step by close to 50 percent.
\paragraph{ISLBM initialization}
\begin{enumerate}
  \item setting Re number and relaxation factor 
  \item calculate relaxation time : $$\tau = \frac{\Delta t}{\omega}$$
  \item calculate viscosity : $$\nu = c_s^{2}(\tau - 0.5 dt)$$
  \item calculate reference velocity : $$U_{ref} = Re \ nu$$
\end{enumerate}
\paragraph{GILBM initialization}
\begin{enumerate}
  \item setting Re number and relaxation factor 
  \item calculate contravariant velocity (i.e., normalized discrete particle velocity set) : $$e_{\alpha}^{i} = c_{\alpha}^{\beta}\frac{\partial \xi_{i}}{\partial x_{\beta}}$$
  \item calculate the global time step using contravariant velocity : $$\Delta t_{glopbal} = CFL \cdot min_{i,j,k,\alpha}|\frac{1}{e_{\alpha}^{i}|_{j,k}}|$$ 
  \item calculate the relaxation time : $$\tau = \frac{\Delta t_{global}}{\omega}$$
  \item calculate the viscosity  $$\nu = (\tau - 0.5\Delta t_{global})c_s^{2}$$
\end{enumerate}
\end{document}
