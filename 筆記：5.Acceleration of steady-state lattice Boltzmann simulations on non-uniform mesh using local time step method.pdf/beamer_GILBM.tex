...
Extend \textbf{Interpolation-Supplemented LBM (ISLBM)} to general curvilinear coordinates through \alert{conformal mapping}, without changing the lattice system.
\end{block}
...
\textbf{Standard LBE (Cartesian):}
\[
f_{i}(\vec{x} + \vec{c}_{i} \Delta t, t + \Delta t) = f_{i}(\vec{x}, t) + \Omega(f_i, f_i^{eq})
\]
...
\begin{frame}{Contravariant Velocity (Normalized Discrete Velocity)}

\begin{block}{Definition}
Transform Cartesian lattice velocity to curvilinear coordinates:
\[
\boxed{\vec{e}_{\alpha}^{\ j} = c_{\alpha}^{i} \frac{\partial \xi_{j}}{\partial x_{i}}}
\]
\end{block}

\textbf{Physical Interpretation:}
\[
\vec{e}_{\alpha} = \underbrace{c_{\alpha}^{i}}_{\text{lattice velocity}} \cdot \underbrace{\vec{g}_{i}(\vec{\xi})}_{\text{tangent vector}} \cdot \underbrace{\frac{\Delta x}{\Delta t}}_{\text{lattice speed} \approx 1}
\]
