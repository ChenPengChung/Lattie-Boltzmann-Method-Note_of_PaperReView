...
we can say "position information only have position variable located at the grid node, so we can simply to transform by substituting
.
\begin{equation}\begin{aligned}
...
In general curvilinear coordinate, the variation of position vector or velocity or any physical variable related length dimension need to condsider transform factor for basis transformation. 
This is complexity for analiyzing the motion of particles. Let's define normalized discrete velocity set. 
\begin{equation}\begin{aligned}
...
\end{aligned}\end{equation}
where the vector $\vec{e}_{\alpha}$ is the non-dimension discrete particle velocity set, and vector $\vec{g}_{i}$ is the tangent vector related to the curvature at the position. $\frac{\Delta x}{\Delta t}$ is the lattice speed, and its function in this definition is velocity dimension.
...
\end{aligned}\end{equation}
where $x$ represent the index of Cartesian coordinate and $\xi$ is the index of general curvilinear coordinate, and the differential represent the basis transform from Cartesian coordinate to curve coordinate. 
...
we can see the essential issue is compute the integration of the dis crete particle velocity to know the  position where is post streaming position and streaming length. The length of the streaming step is defined as  below : 
\begin{equation}\begin{aligned}
...
\noindent  using 2nd  Runge-Kutta method to solving the integration, then it is apart into two step : 
...
\end{aligned}\end{equation}
The result of discrete integration \eqref{eq:RK} have 2nd order accuracy in time space. The figure show that the method of 2nd Runge-Kutta to treat integration.
\begin{figure}[H]
\centering
...
The discrete process is little similar as the process of distribution for time-space discretizing. However, the term discretized is different, one is time integration of distribution and the other is time integration of particle path. 
In curvilinear coordinate , we have 
\begin{equation}\begin{aligned}
  f_{\alpha} (\vec{\xi} , t ) = f_{\alpha}^{\star}(\vec{\xi}-\Delta \vec{\xi}_{\alpha} , t)
\end{aligned}\end{equation}
where$\Delta \vec{\xi}_{\alpha}$ the change of position have 2nd order accuracy in time space, and R.H.S term is post-collision function.
\paragraph{Accuracy effect in time space for lattice bolrtzmann equation - BGK operator}
\noindent LBGK in curvilinear coordinate can be rewrite below : 
...
\end{aligned}\end{equation}
and we can define a material derivative for discrete-velocity Boltzmann equation 
\begin{equation}\begin{aligned}
   \mathcal{D}_{t} = \partial_{t} + \frac{\delta\vec{\xi}_{\alpha}}{\delta t} \cdot \vec{\nabla}_{\xi}
\end{aligned}\end{equation}\
we call above operator as "discrete-velocity material derivation".
...
\end{aligned}\end{equation}
left hand side and right hand side are both divided by $\Delta t$, can reover N-S equation in differential form.
\begin{equation}\begin{aligned}
...
For collision and straming in mesoscopic space, and advevtion and diffusion in macroscpic space, we have three types of time scale :
\begin{enumerate}
...
\end{enumerate}
If K = 0.01, then 
so we can define three types of time variables : (the value is about $K^{(0)}, K^{(1)}$, $K^{(2)}$)\\
...
\end{equation}
Therefore, the differential about time variable is :
\begin{equation}
...
\begin{equation}\begin{aligned}
     ...
     \Rightarrow \vec{\nabla}_{\vec{\xi}} &= K \vec{\nabla}_{\vec{\xi}^{(1)}}
\end{aligned}\end{equation}
Finally, for three types of time variable, multi-scale expansion of dis distribution funtion : 
\begin{equation}
   f_{i} = f_{i}^{eq} + K f_{i}^{(1)}  + K^{2}f_{i}^{(2)} + \dots
\end{equation} 
We have to known that time scale is a domain for time space like $K^{(0)}, K^{(1)}$, and the element of the time scale is a variable, and their scale have different property. 
...
\end{aligned}\end{equation}
Using particle material derivation : 
\begin{equation*}
  \mathcal{D}_{t} = \partial_{t} + \vec{e}_{\alpha} \cdot \vec{\nabla}
\end{equation*}
and Taylor expansion to analiyzing the left hand side of the LBGK :
\begin{equation}\begin{aligned}
...
\end{aligned} \end{equation}
substitue above equation using the discrete velocity Boltzmann equation : 
...
\end{aligned}\end{equation}
For analiyzing the equation above, review the multiple scale relation as shown below: 
\begin{equation}\begin{aligned}
