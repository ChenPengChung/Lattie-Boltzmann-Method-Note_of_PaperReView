...
\end{aligned}\end{equation}
where$\Delta \vec{\xi}_{\alpha}$ the change of position have 2nd order accuracy in time space, and the R.H.S term is the post-collision function.
...
\begin{equation}
    f_{\alpha}^{eq} = w_{\alpha} \rho \left(1 +\frac{c_{\alpha}^{i}u_{i}}{c_s^{2}} + \frac{u_{i}u_{j}\left(c_{\alpha}^{i} c_{\alpha}^{j} - c_s^{2} \delta^{ij}\right)}{2c_{s}^{4}}\right)
\end{equation}
...
\begin{equation}\begin{aligned}
  \frac{\partial f^{eq}}{\partial u_{i}}(\vec{r}, t) = \omega(\vec{r})\rho \left[\frac{3c_{\alpha}^{i}}{c_{s}^{2}} + \frac{u_{j}\left(9c_{\alpha}^{i}c_{\alpha}^{j} - c_s^{2}\delta^{ij}\right)}{2c_s^{4}}\right]  
\end{aligned}\end{equation} 
\subsection{Algorithm}5g
