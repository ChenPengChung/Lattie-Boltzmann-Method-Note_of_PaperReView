...
This note will promote the Interpolation-supplement Lattice Boltzmann Method for using in curvilinear coordinate. However, The first stratgy is transform Cartesian coordinate to general curvilinear coordinate through comformal mapping.  
Different from the preview paper, this method is extend $\textbf{ISLBM}$ ,and do not change the lattice system to curve motion. 
Notice, because exist the transform based on transformation, so we still compute the curve path for pasrticle in computational domain. 
