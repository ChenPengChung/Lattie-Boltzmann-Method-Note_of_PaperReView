...
The discrete process is little similar as the process of distribution for time-space discretizing. However, the term diecretized is different, one is time integration of distribution and the other is time integration of particle path. 
...
\begin{equation}\begin{aligned}
  &\left.\left(\partial_{t} + \frac{\delta\vec{\xi}_{\alpha}}{\delta t} \cdot \vec{\nabla}_{\xi_{\alpha}}\right)f_{\alpha} \Delta t  + \frac{1}{2}\left(\partial_{t} + \frac{\delta\vec{\xi}_{\alpha}}{\delta t} \cdot \vec{\nabla}_{\xi_{\alpha}}\right)^2f_{\alpha} \Delta t ^{2}\right |_{(\vec{\xi} - \Delta \vec{\xi}_{\alpha}, t )}= \omega (f_{\alpha}(\vec{\xi} - \Delta \vec{\xi}_{\alpha}, t ) - f_{\alpha}^{eq}(\rho , \vec{u} , t)) \\[1.5ex]
\end{aligned}\end{equation}
left hand side and right hand side are both divided by $\Delta t$, can reover N-S equation in differential form.
\begin{equation}\begin{aligned}
  &\left.\left(\partial_{t} + \frac{\delta\vec{\xi}_{\alpha}}{\delta t} \cdot \vec{\nabla}_{\xi_{\alpha}}\right)f_{\alpha} + \frac{1}{2}\left(\partial_{t} + \frac{\delta\vec{\xi}_{\alpha}}{\delta t} \cdot \vec{\nabla}_{\xi_{\alpha}}\right)^2f_{\alpha} \Delta t \right |_{(\vec{\xi} - \Delta \vec{\xi}_{\alpha}, t )} + O(\Delta t^{2})= \omega (f_{\alpha}(\vec{\xi} - \Delta \vec{\xi}_{\alpha}, t ) - f_{\alpha}^{eq}(\rho , \vec{u} , t)) \\[1.5ex]
\end{aligned}\end{equation}
