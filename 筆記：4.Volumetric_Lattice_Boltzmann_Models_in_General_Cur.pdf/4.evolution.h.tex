...
\begin{equation}\begin{aligned}
  &\sum_{\alpha_{0}}^{q-1} \delta N_{\alpha}(q , t) = \sum_{\alpha_{0}}^{q-1}N_{\alpha}(q  , t) - N_{\alpha}^{\star} (q-1 , t-1)\\[1.5ex]
  &= \sum_{\alpha_{0}}^{q-1}  N_{\alpha}(q  , t) - \sum_{\alpha_{0}}^{q-1} N_{\alpha}^{\star} (q-1 , t-1)
= 0 \end{aligned}
\end{equation} 
...
\begin{equation}\begin{aligned}
   \vec{g}^{1} = \frac{\vec{g_{2}}\cross \vec{g}_{3}}{(\vec{g_{2}}\cross \vec{g_{3}}) \cdot \vec{g}_{1}} 
\end{aligned}\end{equation}
Substitute the vector in equation \eqref{eq:Theta}
  \begin{equation}\label{eq:Theta}
   \vec{\Theta}_{i}^{j} (\vec{q},t) = \left[\vec{g}_{i}(\vec{q} , t) - \vec{g}_{i}(\vec{q} - \vec{\tilde{c}_{i}} \Delta t , t)\right] \cdot \frac{\vec{g_{2}}\cross \vec{g}_{3}}{(\vec{g_{2}}\cross \vec{g_{3}}) \cdot \vec{g}_{1}}
  \end{equation}
\begin{definition}
  \begin{equation}\begin{aligned}
    &\vec{M_{Loss}} (q,t) \equiv
     \sum_{\alpha = 1}^{q} N_{\alpha}(q,t)\vec{e_{\alpha}}(q,t) - N_{\alpha}^{\star}(q-1,t-1)\vec{e_{\alpha}}(q-1,t-1)\\[1.5ex]
    &\vec{M_{Loss}} (q+1,t+1) \equiv
     \sum_{\alpha = 1}^{q} N_{\alpha}(q+1,t+1)\vec{e_{\alpha}}(q+1,t+1) - N_{\alpha}^{\star}(q,t)\vec{e_{\alpha}}(q,t)\\[1.5ex]
  \end{aligned}\end{equation}
\end{definition}
For curvilinear coordinate, the loss of momentum always exists in streaming step. The basic reason of the loss of macroscopic value is: 
\begin{equation}
...
\begin{definition}
  \begin{equation}\begin{aligned}
    & \mathcal{J}(q) \vec{\chi}^{in}(q,t) \equiv
     \sum_{\alpha = 1}^{q} N_{\alpha}(q,t)\left(\vec{e_{\alpha}}(q,t) - \vec{e_{\alpha}}(q-1,t-1) \right)\\[1.5ex]
     & \mathcal{J}(q) \vec{\chi}^{out}(q,t) \equiv
     \sum_{\alpha = 1}^{q} \left(\vec{e_{\alpha}}(q+1,t+1) -\vec{e_{\alpha}}(q,t)\right) N_{\alpha}^{\star}(q,t)\\[1.5ex]
  \end{aligned}\end{equation}
\end{definition}
