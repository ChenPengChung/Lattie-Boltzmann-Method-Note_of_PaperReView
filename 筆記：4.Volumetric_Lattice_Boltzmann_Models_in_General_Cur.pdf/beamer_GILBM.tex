...
\begin{frame}{Path Integration: Accuracy Analysis}
\textbf{material derivative for the discrete-velocity Boltzmann equation:}
\begin{equation*}
...
\begin{equation*}
  \left.\left(\partial_{t} + \frac{\delta\vec{\xi}_{\alpha}}{\delta t} \cdot \vec{\nabla}_{\xi}\right)f_{\alpha} + \frac{1}{2}\left(\partial_{t} + \frac{\delta\vec{\xi}_{\alpha}}{\delta t} \cdot \vec{\nabla}_{\xi}\right)^2f_{\alpha} \Delta t \right|_{(\vec{\xi} - \Delta \vec{\xi}_{\alpha}, t )} + O(\Delta t^{2})= \omega (f_{\alpha}(\vec{\xi} - \Delta \vec{\xi}_{\alpha}, t ) - f_{\alpha}^{eq}(\rho , \vec{u} , t)) 
\end{equation*}
Therefore, we
must use a method that provides higher-order temporal accuracy for the path integration, ensuring that
the differential term of the velocity-discrete Boltzmann equation has at least second-order temporal
accuracy.
...
\begin{frame}{Boundary Condition Overview}
\textbf{Strategy:} In GILBM, we enforce boundary conditions by constraining macroscopic parameters at boundary nodes (nodes between solid and fluid regions).

\vspace{0.3cm}
\textbf{Distribution function at boundaries:}
\begin{equation}
\begin{aligned}
  \left.f_{\alpha}\right|_{\text{b.c.}} &= f_{\alpha}^{\ \text{eq}} + f_{\alpha}^{\ (1)}\\[0.5ex]
  &= f_{\alpha}^{\ \text{eq}} + \omega \Delta t \, f_{\alpha}^{\ \text{eq}} \left(\frac{(c_{\alpha}^{i} - u_{i})(c_{\alpha}^{j} - u_{j})}{c_s^{2}}\frac{\partial u_{i}}{\partial x_{j}} - \frac{\partial u_{j}}{\partial x_{j}}\right)
\end{aligned}
\end{equation}
...
\begin{equation*}
\frac{\partial u_{i}}{\partial x_{j}} = \frac{\partial u_{i}}{\partial \xi_{k}} \frac{\partial \xi_{k}}{\partial x_{j}}
\end{equation*}
...
\begin{frame}{First-Order Non-Equilibrium Distribution (1/2)}
\textbf{From Multi-Scale Analysis:}

\begin{equation}
\boxed{
f_{\alpha}^{\ (1)} = -\omega \delta t\left(\partial_{t}^{(1)}+ \vec{e}_{\alpha} \cdot \vec{\nabla}^{(1)}\right) f_{\alpha}^{\ \text{eq}}
}
\end{equation}

\vspace{0.2cm}
\textbf{Chain Rule for $f_{\alpha}^{\ eq}$ Derivatives:}
\begin{align*}
\partial_{t}^{(1)} f_{\alpha}^{\ eq} &= \frac{\partial f_{\alpha}^{\ eq}}{\partial u_{i}}\partial_{t}^{(1)} u_{i} + \frac{\partial f_{\alpha}^{\ eq}}{\partial \rho}\partial_{t}^{(1)} \rho \\[1ex]
\vec{\nabla}^{(1)}f_{\alpha}^{\ eq} &= \frac{\partial f_{\alpha}^{\ eq}}{\partial u_{i}}\vec{\nabla}^{(1)} u_{i} + \frac{\partial f_{\alpha}^{\ eq}}{\partial \rho} \vec{\nabla}^{(1)} \rho
\end{align*}

\vspace{0.2cm}
\textbf{Partial Derivatives of $f_{\alpha}^{eq}$:}
\[
\frac{\partial f_{\alpha}^{\ eq}}{\partial u_{i}} \approx \frac{e_{\alpha}^{i} - u_{i}}{c_{s}^{2}} f_{\alpha}^{\ eq}, \quad
\frac{\partial f_{\alpha}^{\ eq}}{\partial \rho} = \frac{1}{\rho} f_{\alpha}^{\ eq}
\]

\end{frame}

\begin{frame}{First-Order Non-Equilibrium Distribution (2/2)}
\textbf{Conservation Laws ($K^{(1)}$ scale):}
\[
\partial_{t}^{(1)} \rho = -\vec{\nabla}^{(1)} \cdot (\rho\vec{u}), \quad
\partial_{t}^{(1)} \vec{u} = -\vec{u}\cdot\vec{\nabla}^{(1)}\vec{u} - \frac{1}{\rho}\vec{\nabla}^{(1)} p
\]

\vspace{0.2cm}
\textbf{Boundary Assumptions:}
\begin{enumerate}
...
\vspace{0.2cm}
\textbf{Final Result --- Distribution at Boundary:}
\[
...
\vspace{0.1cm}
\small{This allows boundary conditions to be enforced through \alert{macroscopic variables} only.}
\end{frame}
...
\begin{enumerate}
    ...
    \]
    \item global time step : ($\Delta t_{g}$):
    \[
    \Delta t_{g} = CFL \cdot min_{i,j,k,\alpha}|\frac{1}{e_{\alpha}^{i}|_{j,k}}|
    \]
...
\begin{enumerate}
    \item Interpolation : 
    {\footnotesize
    \begin{equation}\begin{aligned}\label{eq:eq1}
&f_{i}(\vec{r},t) = f_{i}(x_{i} + \delta x, y_{j} + \delta y, z_{k} +\delta z, t)\\
&=\sum_{m = -3}^{3}\sum_{n = -3}^{3}\sum_{q = -3}^{3} 
f_{i}(x_{i + m}, y_{j + n}, z_{k + q}, t) \cdot \\
&\prod_{\substack{\dot{m} = -3 \\\dot{m} \neq m}}^{3} \frac{\delta x - \dot m}{m - \dot m }
\cdot   \prod_{\substack{\dot{n} = -3 \\\dot{n} \neq n}}^{3} \frac{\delta y - \dot n}{ n - \dot n }
\cdot   \prod_{\substack{\dot{q} = -3 \\\dot{q} \neq q}}^{3} \frac{\delta z - \dot q}{ q - \dot q }
\end{aligned}\end{equation}
}
    \item \textbf{Streaming:}
    $f_i(\vec{\xi}) = f_i^{\ \star}(\vec{\xi} - \Delta\vec{\xi}_i)$
    (with interpolation)
    \vspace{0.5ex}
    \item \textbf{Boundary:} Apply $f_{\alpha}|_{bc}$
    \vspace{0.5ex}
    \item \textbf{Collision:}
    $f_i^{\ \star} = f_i + \omega(f_i^{\ eq} - f_i)$
    \vspace{0.5ex}
...
\begin{block}{GILBM: Coordinate Transform:}
\vspace{0.5ex}
\begin{enumerate}
    \item \textbf{Curvarture} :  Jacobian relation maps Cartesian $\leftrightarrow$ Curvilinear, and pre-calculates the coefficients.
    \item \textbf{Discrete Velocity} : Mulpling the transform coefficient to get contravariant velocity in curvilinear coordinates.
    \item \textbf{Bouncdary Conditon} : Using the macroscopic parameters to enforce the limitation to distriobution function at the boundary nodes.
    \end{enumerate}
\end{block}

\vspace{0.3cm}
\textbf{Limitation:} MRT operator extension to curvilinear coordinates not yet developed. Furthermore, we can see that this method essentially still operates in the curvilinear coordinate system, but transforms the computational domain to appear as a Cartesian grid. However, many other aspects still require coordinate transformation.
